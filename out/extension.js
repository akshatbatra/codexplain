...
function activate(context) {
    ...
    let activeExplainPanel = undefined; // Track the active explain panel
    ...
    function updateDecorations() {
        ...
        const buttonRange = new vscode.Range(endPos, new vscode.Position(endPos.line, Math.min(endPos.character + 20, activeEditor.document.lineAt(endPos.line).range.end.character)));
    ...
    let disposable = vscode.commands.registerCommand('codeExplain.explainCode', async () => {
        ...
        try {
            ...
            const explanationData = await (0, voicePlayer_1.getCodeExplanation)(selectionText);
            ...
            if (activeExplainPanel && !activeExplainPanel.dispose) {
                ...
                activeExplainPanel.webview.html = (0, voicePlayer_1.getWebviewContent)(selectionText, localServerAddress, explanationData);
            }
            else {
                // Create a new panel if none exists
                activeExplainPanel = vscode.window.createWebviewPanel('codeExplainPanel', 'CodeXplain', vscode.ViewColumn.Beside, {
                    enableScripts: true,
            ...
            activeExplainPanel.webview.html = (0, voicePlayer_1.getWebviewContent)(selectionText, localServerAddress);
        }
    });
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('codeExplain.clickDecoration', (textEditor, edit) => {
        vscode.commands.executeCommand('codeExplain.explainCode');
    }));
    ...
    const getExplanationCommand = vscode.commands.registerCommand('codexplain.getExplanation', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage('Analyzing code with IBM Granite...');
            const explanationData = await (0, voicePlayer_1.getCodeExplanation)(selectedText);
            if (activeExplainPanel) {
                activeExplainPanel.webview.html = (0, voicePlayer_1.getWebviewContent)(selectedText, 'http://127.0.0.1:3000/aiVoice', explanationData);
            }
        }
        catch (error) {
